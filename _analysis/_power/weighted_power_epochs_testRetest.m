...
% test rest subjects
subvec = [1,11,3,12];
for ss=1:length(subvec)
    load([d(subvec(ss)).name]);
    ...
    for ff=1:2 % Frequency loop
        ...
        for kk=1:4 % condition loop
            ...
            for ee = e_step:e_step:length(find(data_all.trialinfo==trial_id)) %epoch loop
                e_idx = e_idx+1;
                ...
                rejt_ratio(ss,kk,ee) = reject_epoch;%*100/size(epoched_data,1);
        ...
        for cond = 1:4 %looping for test-retest
            ...
            for n=1:N(ss)
                ...
                F_crit{ff} = finv(0.99,2,2*length(bg_freq))
            end
...
%% test retest plot
close all
colorcodes = cbrewer('qual','Set1',4)
vectypes = {'-','--'}
...
for k=1:2
    t=[1:N(1+2*(k-1))]*tvec;
    diff_pow = squeeze(cell2mat(pow_sig(1+2*(k-1),ff,cvec(cc),:)))-squeeze(cell2mat(pow_sig(2+2*(k-1),ff,cvec(cc),:)));%./squeeze(mean(cell2mat(pow_sig(1+2*(k-1):2+2*(k-1),ff,cvec(cc),N(1+2*(k-1)))),1));
p(k,cc) = plot(t,diff_pow,vectypes{cc},'color',colorcodes(k,:),'linewidth',2);
hold on
end
end
xlabel('Stim time (s)')
ylabel('Difference amplitude [\mu V /Hz]')
hleg = legend([p(1),p(2),p(1,2),p(2,2)],'S1, 25%','S2, 25%','S1, 85%','S2, 85%')
hleg.Box = 'off'
title([num2str(fid(ff)),' Hz']);
set(gca,'fontsize',16)
box off
grid on
set(gcf,'Position',[1297         589         315         420]);
end
...
pow_sig = cellfun(@minus,Psn,Pn,'Un',0);
pow_noise = Pn;
for ii=1:length(subvec) %number of subs
    
    for f=1:2 %frequency
        figure(f)
        for cc=1:4 %condition
            if mod(ii,2)
            p(cc)=plot(ii,SNR{ii,f,cc,N(ii)},[colorcodes{cc}, 'x'])
            else
