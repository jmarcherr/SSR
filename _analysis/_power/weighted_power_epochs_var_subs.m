...
for ii=1:length(d) %number of subs
    ...
    for f=1:2 %frequency
        ...
        legend([p(1),p(2),p(3),p(4)],'25% mod, 40dB','85% mod, 40dB','25% mod, 75dB','85% mod, 75dB')
        xlim([0 length(d)+1])
...
close all
            b=bar(1:4,squeeze(mean(snr_con(:,1:4,:,:),1)))
            hold on

            
            set(gca,'xtick',1:4,'xticklabel',{'25% mod, 40dB','85% mod, 40dB','25% mod, 75dB','85% mod, 75dB'})
            legend(b(1),'4Hz','207Hz')
            ylabel('SNR dB')
            xlabel('Condition')
            
            for cc=1:4
                scatter(ones(1,7)*cc-0.1,squeeze(snr_con(:,cc,1,:)),'b')
                scatter(ones(1,7)*cc+0.1,squeeze(snr_con(:,cc,2,:)),'r')
            end
